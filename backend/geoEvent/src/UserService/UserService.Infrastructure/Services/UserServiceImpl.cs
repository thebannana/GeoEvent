...
public class UserServiceImpl : IUserService
{
    ...
    public async Task<ServiceResult<UserProfileDto>> GetProfileAsync(int userId)
    {
        ...
        if (user is null) return ServiceResult<UserProfileDto>.NotFound($"User {userId} not found.");
    ...
    public async Task<ServiceResult<UserProfileDto>> UpdateProfileAsync(int userId, UpdateProfileDto request)
    {
        ...
        if (request.PhoneNumber is not null) user.Person!.PhoneNumber = request.PhoneNumber;
        if (request.ImageUrl is not null) user.Person!.ImageUrl = request.ImageUrl;
        if (request.CityId is not null) user.Person!.CityId = request.CityId;
    ...
    public async Task<ServiceResult<PagedResult<ReportResponseDto>>> GetAllReportsAsync(ReportStatus? status, int page, int pageSize)
    {
    ...
    public async Task<ServiceResult<ReportResponseDto>> ResolveReportAsync(int reportId, ResolveReportDto dto, int resolvedById)
    {
        var report = await _userRepository.GetReportByIdAsync(reportId);
        if (report is null) return ServiceResult<ReportResponseDto>.NotFound("Report not found.");
    ...
    public async Task<ServiceResult<PublicUserProfileDto>> GetPublicProfileAsync(int userId)
    {
        var user = await _userRepository.GetPublicByIdAsync(userId);
        if (user is null) return ServiceResult<PublicUserProfileDto>.NotFound($"User {userId} not found.");
        return ServiceResult<PublicUserProfileDto>.Ok(MapPublic(user));
    }
    ...
    public async Task<ServiceResult<List<PublicUserProfileDto>>> GetPublicProfilesAsync(IEnumerable<int> userIds)
    {
        ...
        return ServiceResult<List<PublicUserProfileDto>>.Ok(users.Select(MapPublic).ToList());
    }
    ...
    private static PublicUserProfileDto MapPublic(User user) => new()
    {
        UserId = user.PersonId,
        Username = user.Username,
        DisplayName = $"{user.Person!.FirstName} {user.Person.LastName}".Trim(),
